...
sub_path= settings.RUN_ON_SUBPATH or ""
login=os.path.join(sub_path,'login')
...
def detail(request, manifest_name):
    ...
    if request.method == 'GET':
        ...
        c = RequestContext(request, 
            ...
            'page': 'manifests'})
        c.update(csrf(request))
        return render_to_response('manifests/detail.html', c,context_instance=RequestContext(request))
